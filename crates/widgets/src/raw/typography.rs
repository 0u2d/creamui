...
/// logical pixels on every side. Uses the same bundled font as [`RawText`] —
/// no monospace face ships with CreamUI, so column alignment is only
/// approximate.
pub struct RawPre {
    pub style: Style,
    pub text: String,
...
impl Widget for RawPre {
    ...
    fn measure(&self) -> Option<creamui_core::MeasureFn> {
        ...
        Some(Box::new(move |known_dimensions, available_space| {
            ...
            let natural_height =
                crate::text_metrics::content_height(&text, font_size, content_width)
                    + padding * 2.0;
    ...
    fn measure(&self) -> Option<creamui_core::MeasureFn> {
        ...
        Some(Box::new(move |known_dimensions, available_space| {
            ...
            let (natural_width, natural_height) =
                crate::text_metrics::measure_weight(&text, font_size, max_width, false);
            creamui_core::layout::Size {
