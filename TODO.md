...
1. **`.font_family()` is only wired into `Text`/`Heading`.** Calling it on `TextInput`/`TextArea`/`Link`/`Pre` compiles but silently does nothing — it's not threaded through their paint/measure paths. Silent no-op is the worst kind of bug here.
...
4. **Benchmark overlay (`CUI_ENABLE_BENCHMARK`) has no RAM/CPU sampling on Windows.** Linux (`/proc/self/*`) and macOS (`getrusage`) are implemented; Windows falls back to "n/a". Not blocking, just incomplete.
